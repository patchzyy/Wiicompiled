...
internal static class Program
{
    ...
    {
        // Checked anywhere in argv, not just args[0]: AppRun (Launcher/build-appimage.sh) and the
        // sandbox entrypoint (Launcher/build-flatpak.sh) prepend --workspace <cache> ahead of
        // whatever the caller passed, so these can't assume position 0.
    ...
    {
        // AppRun (Launcher/build-appimage.sh) and the sandbox entrypoint (Launcher/build-flatpak.sh)
        // invoke this as `wiicompiled-setup --workspace <cache> <command> [options]` - a global flag
        // ahead of the subcommand - so the command word is whichever token isn't part of a
        // --flag/value pair, not strictly args[0].
    ...
    /// of order - a --flag may appear before or after the command (see the AppRun/entrypoint caller
    /// note in RunAsync). The first token that is neither a --flag nor a value already consumed by the
